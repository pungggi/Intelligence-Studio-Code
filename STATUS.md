...
| **Phase 6** | Marketplace & discovery (registry server, `.ccext` install, Native badge, ed25519 signatures) | **Done** (2026-08-25) — deployment + Native tab are backlog, see [phase-6-marketplace.md](docs/plans/impl/phase-6-marketplace.md) |
...
## WASM Extension Host (parallel track) — PHASES 1–6 COMPLETE
...
### Marketplace (Phase 6 — complete)

`tools/marketplace-server` — standalone axum registry implementing the publish protocol above plus read endpoints (`/api/v1/extension/{id}`, `/latest`, `/{version}/download` with `x-corecode-sha256`, `/search`, `/health`). Filesystem store (`index.json` + immutable `packages/<id>/<version>.ccext`), atomic index writes, immutable versions (409 on re-publish), SHA-256 recorded at publish and verified by the client before install. 15 unit/integration tests.

**ed25519 signatures (TOFU key pinning)**: `cargo corecode keygen` generates a signing keypair; `publish` signs the package when `--signing-key`/`CORECODE_SIGNING_KEY` is set (`X-CoreCode-Signature` + `X-CoreCode-Pubkey` headers). The registry verifies signatures at publish, pins the first key per extension id, and rejects mismatches/unsigned re-publishes (403). `install_native_extension` verifies the downloaded bytes against the signature and pinned key after the SHA-256 check. Unsigned legacy entries still install (SHA-256 always applies).

App side: `marketplace.rs` gains `ccext_get` / `ccext_download` (HTTPS-only outside loopback/LAN, URLs built from validated segments, 50MB cap) + `verify_ccext_signature`; new commands `marketplace_get_native` + `install_native_extension` (resolves latest, downloads, verifies SHA-256 + signature, unpacks via the existing installer — `.ccext` carries `corecode.toml` at the root and routes to the WASM host). Extensions panel shows a **Native / Node.js host-type badge** (`InstalledExtension.kind`, recomputed from disk on every listing).

Backlog (not blockers): public deployment of `marketplace.corecode.dev`, Native tab in the marketplace UI, key rotation tooling. Details: [phase-6-marketplace.md](docs/plans/impl/phase-6-marketplace.md).
...
| Tools — `cargo-corecode` | Manifest, packaging, publish validation, signing | 10 tests |
| Tools — `marketplace-server` | Store roundtrip/conflict/pinning, API auth/roundtrip/signatures | 15 tests |
...
- [ ] **Marketplace rollout (backlog)** — public deployment of `marketplace.corecode.dev` (DNS + TLS + token), Native tab in the marketplace UI, key-rotation tooling. All Phase 6 protocol features are implemented
